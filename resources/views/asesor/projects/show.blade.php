...
@section('title', 'Web Audit - Detail Tugas')
...
<div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 pb-4 border-b border-slate-100 mb-6">
    <div class="flex items-center space-x-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Detail Tugas Audit</h1>
            <p class="text-[10px] text-slate-500">Kelola dan selesaikan penilaian kapabilitas COBIT 2019.</p>
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
    ...
    <!-- Left Column: Project Summary & Maturity Index -->
    <div class="space-y-6">
        <!-- Summary Card -->
        <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-2xs space-y-4">
            ...
            <div class="space-y-3.5 text-xs">
                ...
                <div>
                    <span class="text-slate-400 block mb-0.5">Tanggal Ditugaskan:</span>
    ...
    <!-- Right Column: COBIT Processes Scope & Workspaces -->
    <div class="bg-white border border-slate-100 rounded-2xl shadow-2xs overflow-hidden lg:col-span-2 space-y-6">
        <div class="px-6 py-5 border-b border-slate-100">
            ...
            <p class="text-[10px] text-slate-400 mt-0.5">Daftar proses tata kelola/manajemen TI yang wajib Anda nilai.</p>
        </div>
        ...
        <div class="p-6 space-y-6 divide-y divide-slate-100">
                ...
                <div class="pt-6 first:pt-0 space-y-4">
                    ...
                    <div class="flex items-start justify-between">
                        ...
                        <div>
                            @if($projectProcess->status === 'completed')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-600 text-[9px] font-bold">Selesai Dinilai</span>
                            @elseif($projectProcess->status === 'in_progress')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-[9px] font-bold">Sedang Diisi</span>
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-600 text-[9px] font-bold">Belum Mulai</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 border border-slate-100 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div class="flex items-center space-x-2.5 w-full sm:w-auto shrink-0 justify-end">
                            ...
                            <a href="{{ route('asesor.projects.workspace', [$project->id, $projectProcess->process_code]) }}" class="inline-flex items-center justify-center px-4 py-2 text-xs font-bold rounded-xl shadow-2xs hover:scale-[1.01] active:scale-[0.99] transition-all min-w-[120px] {{ $projectProcess->status === 'completed' ? 'bg-slate-100 hover:bg-slate-250 text-slate-700' : 'bg-indigo-600 hover:bg-indigo-700 text-white' }}">
                                @if($projectProcess->status === 'completed')
                                    Ulangi Penilaian
                                @elseif($projectProcess->status === 'in_progress')
                                    Lanjutkan Audit
            ...
            @empty
                <div class="py-8 text-center text-slate-400 font-medium text-xs">Belum ada proses COBIT yang diaktifkan untuk Anda audit pada proyek ini.</div>
            @endforelse
        </div>
    </div>
