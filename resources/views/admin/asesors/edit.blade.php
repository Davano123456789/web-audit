...
<div class="flex items-center space-x-3 pb-4 border-b border-slate-100 mb-6">
    ...
    <div>
        <h1 class="text-xl font-bold text-slate-900 tracking-tight">Perbarui Akun Asesor</h1>
        <p class="text-[10px] text-slate-500">Ubah informasi nama, alamat email, atau perbarui kata sandi untuk akun Asesor.</p>
    </div>
</div>
...
<div class="max-w-md mx-auto">
    <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-2xs space-y-6">
        <div>
            ...
            <p class="text-[10px] text-slate-400 mt-0.5">Kosongkan kolom kata sandi jika Anda tidak berniat menggantinya.</p>
        </div>

        <form action="{{ route('admin.asesors.update', $asesor->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <!-- Password Input -->
            <div>
                <label for="password" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wide">Kata Sandi Baru (Opsional)</label>
